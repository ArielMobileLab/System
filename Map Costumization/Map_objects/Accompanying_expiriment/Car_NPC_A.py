...
def Generate_Car1():
    ...
    vehicle_bp = random.choice(blueprint_library.filter('vehicle.carlamotors.carlacola'))

    # Define the spawn location for the vehicle
    spawn_location = carla.Transform(
        carla.Location(x=88, y=17, z=1),  # Initial location
...
def Cars(data):
    ...
    if first_car_cross == True and 130<data.pose.pose.position.x<160 and -333<data.pose.pose.position.y<-323:
        print("car_1")
        Generate_Car1()
