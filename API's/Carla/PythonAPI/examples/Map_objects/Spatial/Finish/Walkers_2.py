...
def first_walker_cross():
    ...
    spawn_point = carla.Transform(carla.Location(x=98.174629, y=8.9, z=0.300000), carla.Rotation(yaw=0.0)) 
    ...
    target_location = carla.Location(x=114.1, y=8.9, z=0.300000) 
    #speed of the walker
...
def secound_walker_cross():
    ...
    # Define the walker's initial location and transform
    spawn_point = carla.Transform(carla.Location(x=324, y=51.1, z=0.300000), carla.Rotation(yaw=90.0))
    ...
    walker_controller.start()
    # Set the walker's target location
    target_location = carla.Location(x=340, y=51.1, z=0.300000)
    #speed of the walker
    walker_controller.set_max_speed(1.2)
...
def Cross_Walker(data):
    ...
    if first_walker_cross_cond == True and 87.4309 < data.pose.pose.position.x < 93.3200:

       print("walker_1")
       first_walker_cross()
       first_walker_cross_cond = False

    if secound_walker_cross_cond == True  and 325<data.pose.pose.position.x<340 and -3.682<data.pose.pose.position.y<4.4960 :
        print("walker_2")
