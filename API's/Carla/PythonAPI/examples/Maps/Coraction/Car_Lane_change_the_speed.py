...
control.throttle = 0.6  # Initial throttle
...
noOfWp = 10000
t = 0
throttle_change_counter = 0
throttle = 0.6  # Initial throttle
while t < noOfWp:
    wp_next = wp.next(5.0)
    if len(wp_next) > 1:
        wp = wp_next[1]
...
# Function to toggle brake lights
def toggle_brake_lights(vehicle, state):
...
while t < noOfWp:
    ...
    time.sleep(0.5)
    t += 1
    throttle_change_counter += 1

    if throttle_change_counter == 20:
        if throttle == 0.6:
            throttle = 0.3
        else:
            throttle = 0.6
    ...
    # Check if the car is slowing down
    if t > 0:
        prev_vf = np.sqrt(prev_vel.x**2 + prev_vel.y**2)
        if vf < prev_vf:
            toggle_brake_lights(vehicle, True)
        else:
            toggle_brake_lights(vehicle, False)

    prev_vel = veh_vel  # Store current velocity for next iteration
