...
def secound_walker_cross():
    ...
    # Define the walker's initial location and transform
    spawn_point = carla.Transform(carla.Location(x=92.779224854, y=-21.6183681488, z=0.300000), carla.Rotation(yaw=0.0))
    ...
    walker_controller.start()
    # Set the walker's target location
    target_location = carla.Location(x=96.174629, y=65.078133, z=0.300000)
...
def Cross_Walker(data):
    ...
    if first_walker_cross_cond == True:
       print("walker_1")
       first_walker_cross()
       first_walker_cross_cond = False

    if secound_walker_cross_cond == True and 115.0<data.pose.pose.position.x<116.722782898 and -130.0<data.pose.pose.position.y<-129.0 :
        print("walker_2")
