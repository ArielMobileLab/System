...
image_pub = rospy.Publisher('/carla/camera/3D', Image, queue_size=10)
bridge = CvBridge()
...
# Desired resolution for the published image
publish_width = 1300
publish_height = 620
...
def pygame_callback(data, obj):
    ...
    # Resize image to the desired resolution for ROS publishing
    resized_img = cv2.resize(img, (publish_width, publish_height))
    
    # Convert image to ROS message
    try:
        ros_image = bridge.cv2_to_imgmsg(resized_img, "rgb8")
...
camera_init_trans = carla.Transform(carla.Location(x=0.12, y=0.0, z=1.6), carla.Rotation(yaw=0.0))
