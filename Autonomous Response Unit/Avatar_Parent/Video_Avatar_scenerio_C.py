...
def odometry_callback(data):
    ...
    # Video 1 condition
    if 200 < x < 220 and -61<y<-51 and not video_flags['video1']:
        ...
        video_path1 = '/home/omer/Desktop/Autonomous Resope Unit/Avatar_Parent/Avatar_video/red_light.mp4'
        play_video(video_path1, window_x, window_y, window_width, window_height)
        video_flags['video1'] = True


    # Video 2 condition
    elif -7 < x < 7 and -110 < y < -50 and not video_flags['video2']:
        timestamp = datetime.now().strftime('%H:%M:%S.%f')
        simulation_time = data.header.seq * 0.033333335071821
    ...
    # Video 3 condition
    elif 31<x<50 and -335<y<-320 and not video_flags['video3']:
        timestamp = datetime.now().strftime('%H:%M:%S.%f')
        simulation_time = data.header.seq * 0.033333335071821
        ...
        Egocar_data["Simulation_time_ROS"] = simulation_time
        Egocar_data["Simulation_time"] = simulationTime
        Egocar_data["Video"] = "T-junction Gap"
        write_to_json(Egocar_data)  # for json


        video_path3 = '/home/omer/Desktop/Autonomous Resope Unit/Avatar_Parent/Avatar_video/barrier.mp4'
    ...
    # Video 5 condition
    elif 275<x<293 and -139<y<-123 and not video_flags['video5']:
        timestamp = datetime.now().strftime('%H:%M:%S.%f')
        simulation_time = data.header.seq * 0.033333335071821
        ...
        Egocar_data["Simulation_time_ROS"] = simulation_time
        Egocar_data["Simulation_time"] = simulationTime
        Egocar_data["Video"] = "passenger_cross"
        write_to_json(Egocar_data)  # for json
        video_path5 = '/home/omer/Desktop/Autonomous Resope Unit/Avatar_Parent/Avatar_video/person_cross.mp4'
        play_video(video_path5, window_x, window_y, window_width, window_height)
        video_flags['video5'] = True


    # Video 5 condition
    elif 290 < x < 310 and -332.00 < y < -323.00 and not video_flags['video6']:
        timestamp = datetime.now().strftime('%H:%M:%S.%f')
        simulation_time = data.header.seq * 0.033333335071821
