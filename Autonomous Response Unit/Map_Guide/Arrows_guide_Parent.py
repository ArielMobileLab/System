...
def Arrows(data):
    ...
    if Map_type == 'Guide_Parent':
        ...
        if -116.1248 < data.pose.pose.position.y < -90.128 and 88.2840 < data.pose.pose.position.x < 94.0:
    ...
    if Map_type == 'Guide_Parent_C':
        ...
        if seven_errow == True and 329.4758<data.pose.pose.position.x<344.0 and -230.6557<data.pose.pose.position.y< -220.9451 :
            print("6")
            Egocar_data = OrderedDict()
            Egocar_data["Type"] = "Video_Status:"
            Egocar_data["World_Time"] = timestamp
        ...
        if  A3 == True and -332 < data.pose.pose.position.y < -324 and 50 < data.pose.pose.position.x < 83:
                    print("11")
                    if Brake_blackWindow == True:
                        blackWindow(black_window,24)
